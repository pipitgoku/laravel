		...
		<div id="frame-tabel">
			...
			<div class="table-responsive">
				<table class="table table-single-select datatable-pagination" id="tabel-data" width="100%">
					<thead>
						<tr>
							<th id="golongan_id_table">ID</th>
							<th id="golongan_cd_table">Kode</th>
							<th id="golongan_nama_table">Golongan</th>
							<th id="pangkat_nama_table">Pangkat</th>
				...
				<div class="row">
					<div class="col-md-2">
						<div class="form-group form-group-float">
							...
							<input type="text" name="golongan_cd" class="form-control" required="" placeholder="" aria-invalid="false">
						</div>
					</div>
					<div class="col-md-4">
						<div class="form-group form-group-float">
							<label class="form-group-float-label is-visible">Golongan <span class="text-danger">*</span></label>
							<input type="text" name="golongan_nama" class="form-control" required="" placeholder="" aria-invalid="false" maxlength="50">
						</div>
					</div>
					<div class="col-md-4">
						<div class="form-group form-group-float">
							<label class="form-group-float-label is-visible">Pangkat <span class="text-danger">*</span></label>
							<input type="text" name="pangkat_nama" class="form-control" required="" placeholder="" aria-invalid="false" maxlength="50">
						</div>
					</div>
				</div>
...
<script>
    ...
    var baseUrl = '{{ url("sistem/golongan/") }}';
    ...
    $(document).ready(function(){
		...
		tabelData = $('#tabel-data').DataTable({
			...
			buttons: [
				...
				},
				{
					...
					customize: function (doc) {
						//--Header & Parameter
						var reportTitle =  'Data Golongan/Pangkat';

						//--Full width table
						//doc.content[1].table.widths =Array(doc.content[1].table.body[0].length + 1).join('*').split('');
						doc.content[1].table.widths = [75,100,200];
            ...
            columns: [
                { data: 'golongan_id', name: 'golongan_cd', visible:false },
				{ data: 'golongan_cd', name: 'golongan_cd', visible:true },
                { data: 'golongan_nama', name: 'golongan_nama' },
				{ data: 'pangkat_nama', name: 'pangkat_nama' },
		...
		$('#reload-table').click(function(){
			...
			window.location = '/sistem/golongan/';
		});
        ...
        $('#tambah').click(function()   {
            ...
            $('input[name=golongan_nama]').focus();
            $('#frame-tabel').hide();
            $('#frame-form').show();
        ...
        $('#reset').click(function()   {
            ...
            $('#frame-tabel').show();
            $('#frame-form').hide();
            $('.card-title').text('Data Golongan/Pangkat');
			$('input[name=golongan_cd]').val(rowData['golongan_cd']).prop('readonly',false);
			$('input[name=golongan_nama]').val('');
        });
        ...
        $(document).on('click', '#ubah',function(){
            if (dataCd == null) {
				...
				$('input[name=golongan_cd]').val(rowData['golongan_cd']).prop('readonly',true);
                $('input[name=golongan_nama]').val(rowData['golongan_nama']);
				$('input[name=pangkat_nama]').val(rowData['pangkat_nama']);
    ...
    function reset(type) {
        ...
        $('#frame-tabel').show();
        $('#frame-form').hide();
        $('input[name=golongan_cd]').val('').prop('readonly',false);
        $('input[name=golongan_nama]').val('');
		$('input[name=pangkat_nama]').val('');
        $('.card-title').text('Data Golongan/Pangkat');
    }
</script>
@endpush
